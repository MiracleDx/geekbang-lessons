...
public class Converters implements Iterable<Converter> {
	...
	private void assertConverter(Converter<?> converter) {
		...
		if (converterClass.isInterface()) {
			throw new IllegalArgumentException("The implementation class of Converter must not by an interface!");
		}
		
		if (Modifier.isAbstract(converterClass.getModifiers())) {
			throw new IllegalArgumentException("The implementation class of converter must not be abstrace!");
		}
	}
