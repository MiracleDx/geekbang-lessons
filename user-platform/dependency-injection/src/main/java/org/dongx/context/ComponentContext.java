...
import org.dongx.context.function.ThrowableAction;
	...
	private void injectComponents(Object component, Class<?> componentClass) {
		Stream.of(componentClass.getDeclaredFields())
				.filter(field -> {
					...
					return !Modifier.isStatic(mods) && (field.isAnnotationPresent(Resource.class) || field.isAnnotationPresent(Value.class));
				}).forEach(field -> {
					...
					if (field.isAnnotationPresent(Value.class)) {
						...
						// 查找该字段配置属性
						String property = ServletConfigContextHolder.getInstance().getConfig().getValue(propertyName, String.class);
						if (property == null) {
